...
const schema = z.object({
  ...
  message: z.string().trim().min(10).max(1500), consent: z.literal(true), website: z.string().max(0).optional(),
});

const attempts = new Map<string, { count: number; reset: number }>();
export async function POST(request: NextRequest) {
  const ip = request.headers.get('cf-connecting-ip') || request.headers.get('x-forwarded-for')?.split(',')[0] || 'local';
  const now = Date.now(); const current = attempts.get(ip); if (current && current.reset > now && current.count >= 5) return NextResponse.json({ message: 'Please wait before submitting another request.' }, { status: 429 });
